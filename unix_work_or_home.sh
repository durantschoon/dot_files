...
[[ "$answer" = h* ]] && touch ~/.HOME && echo $file_msg >> ~/.HOME && echo You are now set up for HOME && exit 0
[[ "$answer" = w* ]] && touch ~/.WORK && echo $file_msg >> ~/.WORK && echo You are now set up for WORK && exit 0
